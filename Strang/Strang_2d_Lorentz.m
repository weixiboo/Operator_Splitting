...
LT_J1 = 1/delt + nu./2 + (delt.*omega_0.^2)./16 + delt.*omega_p.^2./(16.*eps_inf);
LT_J2 = 1/delt - nu./2 - (delt.*omega_0.^2)./16 - delt.*omega_p.^2./(16.*eps_inf);
LT_J = LT_J2./LT_J1;
...
for i = 1:NT
    ...
    %Step B: Solve for E and J's (half time step)
    J_x_new = LT_J.*J_x_old - (omega_0.^2./(2*LT_J1)).*P_x_old + eps0*omega_p.^2./(2*LT_J1).*E_x_old;
    J_y_new = LT_J.*J_y_old - (omega_0.^2./(2*LT_J1)).*P_y_old + eps0*omega_p.^2./(2*LT_J1).*E_y_old;

    P_x_new = P_x_old + (delt/4)*(J_x_new+J_x_old);
    P_y_new = P_y_old + (delt/4)*(J_y_new+J_y_old);

    E_x_new = E_x_old - delt./(4*eps0*eps_inf).*(J_x_new + J_x_old);
    E_y_new = E_y_old - delt./(4*eps0*eps_inf).*(J_y_new + J_y_old);

    %Step A: Just plain old Yee shceme
    E_x_new = E_x_new + (delt./(dely*eps0.*eps_inf)).*matrix_padding(diff(H_z_old,1,1),1);
    E_y_new = E_y_new - (delt./(delx*eps0.*eps_inf)).*matrix_padding(diff(H_z_old,1,2),2);
    
    %Step B: Solve for E and J's (half time step)
    J_x_old_temp = J_x_new;
    J_y_old_temp = J_y_new;

    J_x_new = LT_J.*J_x_new - (omega_0.^2./(2*LT_J1)).*P_x_new + eps0*omega_p.^2./(2*LT_J1).*E_x_new;
    J_y_new = LT_J.*J_y_new - (omega_0.^2./(2*LT_J1)).*P_y_new + eps0*omega_p.^2./(2*LT_J1).*E_y_new;

    P_x_new = P_x_new + (delt/4)*(J_x_new+J_x_old_temp);
    P_y_new = P_y_new + (delt/4)*(J_y_new+J_y_old_temp);

    E_x_new = E_x_new - delt./(4*eps0*eps_inf).*(J_x_new + J_x_old_temp);
    E_y_new = E_y_new - delt./(4*eps0*eps_inf).*(J_y_new + J_y_old_temp);
